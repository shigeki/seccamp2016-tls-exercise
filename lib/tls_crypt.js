...
function Encrypt(cipher, key, write_iv, frame, seq) {
  ...
  var encrypted_record_header = Buffer.concat([record_layer.ContentType, record_layer.ProtocolVersion, (new Buffer(2)).fill(0)]);
  ...
  var new_encrypted_record_header = Buffer.concat([record_layer.ContentType, record_layer.ProtocolVersion, (new Buffer(2)).fill(0)]);
  new_encrypted_record_header.writeUInt16BE(encrypted.ciphertext.length + ChaCha20Poly1305.taglength, 3);
...
function Decrypt(cipher, key, write_iv, buf, seq) {
  ...
  var encrypted_data = encrypted_data_and_tag.slice(0, -ChaCha20Poly1305.taglength);
  var tag = encrypted_data_and_tag.slice(-ChaCha20Poly1305.taglength);
  var encrypted_record_header = Buffer.concat([record_layer.ContentType, record_layer.ProtocolVersion, (new Buffer(2))]);
  encrypted_record_header.writeUInt16BE(encrypted_data.length, 3);
  var aad = Buffer.concat([seq, encrypted_record_header]);
  var nonce = BufferXOR(Buffer.concat([(new Buffer(4)).fill(0), seq]), write_iv);
