...
function createServerKeyExchange(connection) {
  ...
  sign.update(buf);
  var signature = sign.sign(connection.leafcert_privatekey);
...
exports.createUnencryptedApplicationData = createUnencryptedApplicationData;
function createUnencryptedApplicationData(connection) {
  var obj = {
  ...
  return TLS.UnencryptedApplicationData(TLS.UnencryptedApplicationData.encode(obj));
}

exports.createEncryptedApplicationData = createEncryptedApplicationData;
function createEncryptedApplicationData(connection) {
  var obj = {
    ContentType: new Buffer('17', 'hex'),
    ProtocolVersion: connection.version,
    Length: null,
  ...
  return TLS.ApplicationData.encode(obj, cipher, key, write_iv, seq);
}