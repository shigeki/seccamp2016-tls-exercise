...
TLS_ST_BEFORE.prototype.read = function(buf, cb) {
  ...
  if (this.is_server && handshake_type === 'ClientHello') {
    ...
    this.connection.state.write(null, cb);
    return true;
  }
...
TLS_ST_SR_CLNT_HELLO.prototype.write = function(buf, cb) {
  ...
  this.connection.state.write(server_hello, cb);
};
...
TLS_ST_SW_SRVR_HELLO.prototype.write = function(buf, cb) {
  ...
  if (handshake_type === 'ServerHello') {
    ...
    this.connection.state.write(Buffer.concat([buf, certificate]), cb);
    return true;
  }
  return false;
};
...
TLS_ST_SW_CERT.prototype.write = function(buf, cb) {
  ...
  this.connection.state.write(Buffer.concat([buf, server_key_exchange]), cb);
  return true;
};
...
TLS_ST_SW_KEY_EXCH.prototype.write = function(buf, cb) {
  ...
  this.connection.state.write(Buffer.concat([buf, server_hello_done]), cb);
  return true;
};
...
TLS_ST_SW_SRVR_DONE.prototype.write = function(buf, cb) {
  var handshake_type = checkHandshakeBuf(buf);
  cb(null, buf);
  return true;
};
