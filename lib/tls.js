...
var Finished = {
  decode: function(buf, cipher, key, write_iv) {
    ...
    assert(record_layer.ContentType.equals(ContentType.Handshake));
    var length = record_layer.Length.readUInt16BE();
    var encrypted_finished_and_tag = data_reader.readBytes(length);
    var encrypted_finished = encrypted_finished_and_tag.slice(0, -ChaCha20Poly1305Decrypt.taglength);
    var tag = encrypted_finished_and_tag.slice(-ChaCha20Poly1305Decrypt.taglength);
    var seq = new Buffer('0000000000000000', 'hex');
    var encrypted_record_header = new Buffer('1603030000', 'hex');
    encrypted_record_header.writeUInt16BE(HandshakeHeaderLength + VerifyDataLength, 3);
    var aad = Buffer.concat([seq, encrypted_record_header]);
    var nonce = BufferXOR(Buffer.concat([new Buffer('00000000', 'hex'), seq]), write_iv);
    var unencrypted_finished = ChaCha20Poly1305Decrypt(aad, key, nonce, encrypted_finished);
    var new_length = (new Buffer(2));
    new_length.writeUInt16BE(unencrypted_finished.plaintext.length);
    var unencrypted_record_header = Buffer.concat([
      record_layer.ContentType,
      record_layer.ProtocolVersion,
      new_length
    ]);
    var obj = UnencryptedFinished.decode(Buffer.concat([unencrypted_record_header, unencrypted_finished.plaintext]));
    assert(tag.equals(unencrypted_finished.tag));
    obj.remaining_buffer = data_reader.peekRemainingPayload();
    return obj;
  },
  encode: function(obj, cipher, key, write_iv) {
    ...
    var aad = Buffer.concat([seq, encrypted_record_header]);
    var nonce = BufferXOR(Buffer.concat([new Buffer('00000000', 'hex'), seq]), write_iv);
    var encrypted = ChaCha20Poly1305Encrypt(aad, key, nonce, unencrypted_finished.slice(5));
    var new_encrypted_record_header = new Buffer('1603030000', 'hex');
    new_encrypted_record_header.writeUInt16BE(HandshakeHeaderLength + VerifyDataLength + ChaCha20Poly1305Encrypt.taglength, 3);
    var buf = Buffer.concat([new_encrypted_record_header, encrypted.ciphertext, encrypted.tag]);
    return buf;
  }
};
...
var ApplicationData = {
  decode: function(buf, cipher, key, write_iv, seq) {
    ...
    var data_reader = new DataReader(buf);
    const record_layer = RecordLayer.decode(data_reader.readBytes(RecordLayerLength));
    assert(record_layer.ContentType.equals(ContentType.ApplicationData));
    var length = record_layer.Length.readUInt16BE();
    var encrypted_data_and_tag = data_reader.readBytes(length);
    var encrypted = encrypted_data_and_tag.slice(0, -ChaCha20Poly1305Decrypt.taglength);
    var tag = encrypted_data_and_tag.slice(-ChaCha20Poly1305Decrypt.taglength);
    var encrypted_record_header = buf.slice(0, 5);
    encrypted_record_header.writeUInt16BE(length - ChaCha20Poly1305Decrypt.taglength, 3);
    var aad = Buffer.concat([seq, encrypted_record_header]);
    var nonce = BufferXOR(Buffer.concat([new Buffer('00000000', 'hex'), seq]), write_iv);
    var plain = ChaCha20Poly1305Decrypt(aad, key, nonce, encrypted);
    assert(tag.equals(plain.tag));
    return {
      ContentType: record_layer.ContentType,
      ProtocolVersion: record_layer.ProtocolVersion,
      Length: record_layer.Length,
  ...
  encode: function(obj, cipher, key, write_iv, seq) {
    assert(cipher === "chacha20", "cipher only supports chacha20");
    var plaintext_length = new Buffer(2);
    plaintext_length.writeUInt16BE(obj.ApplicationData.Plaintext.length);
    var plain_record_header = Buffer.concat([obj.ContentType, obj.ProtocolVersion, plaintext_length]);
    var aad = Buffer.concat([seq, plain_record_header]);
    var nonce = BufferXOR(Buffer.concat([new Buffer('00000000', 'hex'), seq]), write_iv);
    var encrypted = ChaCha20Poly1305Encrypt(aad, key, nonce, obj.ApplicationData.Plaintext);
    var encrypted_length = new Buffer(2);
    encrypted_length.writeUInt16BE(encrypted.ciphertext.length + encrypted.tag.length);
    var encrypted_record_header = Buffer.concat([obj.ContentType, obj.ProtocolVersion, encrypted_length]);
    var buf = Buffer.concat([encrypted_record_header, encrypted.ciphertext, encrypted.tag]);
    return buf;
  }
};
