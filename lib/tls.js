...
const ChaCha20Poly1305taglength = require('./crypto/chacha20_poly1305.js').taglength;
...
var ClientHello = {
  ...
  encode: function(obj) {
    ...
    var handshake_data = Buffer.concat([
    ...
    var handshake_length = new Buffer(3);
    handshake_length.writeUIntBE(handshake_data.length, 0, 3);
    var handshake_buf = Buffer.concat([
      handshake_handshake_type,
      handshake_length,
      handshake_data
    ]);
    var record_layer_length = new Buffer(2);
    record_layer_length.writeUInt16BE(handshake_buf.length);
...
var Certificate = {
  ...
  encode: function(obj) {
    ...
    var handshake_length = new Buffer(3);
    handshake_length.writeUIntBE(handshake_certificates.length, 0, 3);
    var handshake_buf = Buffer.concat([
      handshake_handshake_type,
      handshake_length,
...
var ServerHelloDone = {
  ...
  encode: function(obj) {
    ...
    var handshake = obj.Handshake;
    var handshake_handshake_type = checkBuffer(handshake.HandshakeType);
    var handshake_length = (new Buffer(3)).fill(0);
    var handshake_buf = Buffer.concat([
      handshake_handshake_type,
      handshake_length
    ]);
...
var ClientKeyExchange = {
  ...
  encode: function(obj) {
    ...
    var handshake_ec_public = makeVectorBuffer(handshake.ECPublic, (1 << 8) - 1);
    var handshake_length = new Buffer(3);
    handshake_length.writeUIntBE(handshake_ec_public.length, 0, 3);
    var handshake_buf = Buffer.concat([
      handshake_handshake_type,
      handshake_length,
      handshake_ec_public
...
var UnencryptedFinished = {
  ...
  encode: function(obj) {
    ...
    var handshake_length = new Buffer(3);
    handshake_length.writeUIntBE(handshake_verify_data.length, 0, 3);
    var handshake_buf = Buffer.concat([
      handshake_handshake_type,
      handshake_length,
...
var ApplicationData = {
  decode: function(buf, cipher, key, write_iv, seq) {
    checkBuffer(buf);
    var tag = buf.slice(-ChaCha20Poly1305taglength);
  ...
  encode: function(obj, cipher, key, write_iv, seq) {
    ...
    length.writeUInt16BE(obj.ApplicationData.Plaintext.length);
    var frame = Buffer.concat([obj.ContentType, obj.ProtocolVersion, length, obj.ApplicationData.Plaintext]);
    var encrypted = TLS_CRYPT.encrypt(cipher, key, write_iv, frame, seq);
    return encrypted;
  }
};

var UnencryptedApplicationData = {
  decode: function(buf) {
    checkBuffer(buf);
    var data_reader = new DataReader(unencrypted_buf);
    const record_layer = RecordLayer.decode(data_reader.readBytes(RecordLayerLength));
...
exports.TLS = {
  ...
  UnencryptedApplicationData: UnencryptedApplicationData
};
